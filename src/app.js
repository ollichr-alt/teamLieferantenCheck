...
async function handleSegmentedDataset(fileMap, manifestFile, sourceName = "Datenpaket") {
  try {
    ...
    const result = await importSegmentedDataset(fileMap, manifestFile, progress => {
      setExchangeProgress({
        title: "Segmentierter Datenimport",
        detail: progress.detail,
        percent: progress.percent
      });
    });

    await rememberExchange("dataset", {
      name: `${sourceName} · ${result.datasetVersion}`,
      size: [...fileMap.values()].reduce((sum, f) => sum + (f.size || 0), 0),
      lastModified: Date.now()
    });

    setExchangeProgress({
      title: "Datenimport abgeschlossen",
      detail: `${result.totalRows.toLocaleString("de-DE")} Datensätze wurden aktiviert.`,
      percent: 100
    });

    setTimeout(refresh, 500);
  } catch (error) {
    showExchangeError(error);
  }
}
...
async function handleDataset(file) {
  ...
  try {
    ...
    await importDatasetFile(file, value => {
      setExchangeProgress({
        title: "Datenimport",
        detail: value < 100 ? "Daten werden lokal verarbeitet und sicher aufgebaut…" : "Neuer Datenstand wird aktiviert…",
        percent: value
      });
    });
...
async function handleWorkspace(file) {
  ...
  try {
    ...
    setExchangeProgress({
      title: "Arbeitsstand übernommen",
      detail: result.legacy ? "Älterer Arbeitsstand wurde erfolgreich migriert." : "Arbeitsstand wurde vollständig importiert.",
      percent: 100
    });
...
function bindUi() {
  ...
  document.querySelector("#pick-dataset")?.addEventListener("click", async () => {
    try {
      ...
      if (error?.name !== "AbortError") showExchangeError(error);
    }
  });
  ...
  const runAssistant = async () => {
    ...
    try {
      ...
      if (result.execution?.type === "analysis") {
        container.innerHTML = `<pre>${JSON.stringify(result.execution.data, null, 2)}</pre>`;
      } else {
        container.innerHTML = `<div class="result"><strong>${result.execution?.ok ? "Ausgeführt" : "Nicht erkannt"}</strong><div>${result.execution?.message || result.execution?.target || ""}</div></div>`;
      }
    } catch (error) {
      container.innerHTML = `<div class="result"><strong>${error.code || "AI_ERROR"}</strong><div>${error.message}</div></div>`;
    }
  };
  assistantButton?.addEventListener("click", runAssistant);
...
async function boot() {
  if (!localStorage.getItem("lc-device-id")) {
    localStorage.setItem("lc-device-id", crypto.randomUUID());
  }
